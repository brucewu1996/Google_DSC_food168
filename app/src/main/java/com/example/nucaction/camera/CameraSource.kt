...
class CameraSource(private val graphicOverlay: GraphicOverlay) {
    ...
    /** Returns the preview size that is currently in use by the underlying NucAction.camera.  */
    internal var previewSize: Size? = null
        private set

    /**
     * Dedicated thread and associated runnable for calling into the detector with frames, as the
     * frames become available from the NucAction.camera.
     */
    ...
    /**
     * Map to convert between a byte array, received from the NucAction.camera, and its associated byte buffer.
    ...
    /**
     * Opens the NucAction.camera and starts sending preview frames to the underlying detector. The supplied
    ...
    /**
     * Closes the NucAction.camera and stops sending frames to the underlying frame detector.
     *
     *
     * This NucAction.camera source may be restarted again by calling [.start].
     *
     *
     * Call [.release] instead to completely shut down this NucAction.camera source and release the
    ...
    internal fun stop() {
        ...
        camera?.let {
            ...
            try {
                ...
                Log.e(TAG, "Failed to clear NucAction.camera preview: $e")
            }
            it.release()
            camera = null
        }
    ...
    /** Stops the NucAction.camera and releases the resources of the NucAction.camera and underlying detector.  */
    fun release() {
    ...
    /**
     * Opens the NucAction.camera and applies the user NucAction.settings.
     *
     * @throws IOException if NucAction.camera cannot be found or preview cannot be processed.
     */
    @Throws(IOException::class)
    private fun createCamera(): Camera {
        val camera = Camera.open() ?: throw IOException("There is no back-facing NucAction.camera.")
        val parameters = camera.parameters
        ...
        // Four frame buffers are needed for working with the NucAction.camera:
        //
        //   one for the frame that is currently being executed upon in doing detection
        //   one for the next pending frame to process immediately upon completing detection
        //   two for the frames that the NucAction.camera uses to populate future preview images
        //
        // Through trial and error it appears that two free buffers, in addition to the two buffers
        // used in this code, are needed for the NucAction.camera to work properly. Perhaps the NucAction.camera has one
        // thread for acquiring images, and another thread for calling into user code. If only three
        // buffers are used, then the NucAction.camera will spew thousands of warning messages when detection
    ...
    /**
     * Calculates the correct rotation for the given NucAction.camera id and sets the rotation in the
     * parameters. It also sets the NucAction.camera's display orientation and rotation.
     *
     * @param parameters the NucAction.camera parameters for which to set the rotation.
     */
    ...
    /**
     * Creates one buffer for the NucAction.camera preview callback. The size of the buffer is based off of the
     * NucAction.camera preview size and the format of the NucAction.camera image.
     *
     * @return a new preview buffer of the appropriate size for the current NucAction.camera NucAction.settings.
     */
    private fun createPreviewBuffer(previewSize: Size): ByteArray {
        ...
        check(!(!byteBuffer.hasArray() || !byteBuffer.array()!!.contentEquals(byteArray))) {
            ...
            "Failed to create valid buffer for NucAction.camera source."
        }
     ...
     * available from the NucAction.camera. This is designed to run detection on frames as fast as possible
     * (i.e., without unnecessary context switching or waiting on the next frame).
     *
     *
     * While detection is running on a frame, new frames may be received from the NucAction.camera. As these
    ...
    private inner class FrameProcessingRunnable internal constructor() : Runnable {
        ...
        /**
         * Sets the frame data received from the NucAction.camera. This adds the previous unused frame buffer (if
         * present) back to the NucAction.camera, and keeps a pending reference to the frame data for future use.
         */
        internal fun setNextFrame(data: ByteArray, camera: Camera) {
            synchronized(lock) {
                ...
                if (!bytesToByteBuffer.containsKey(data)) {
                    Log.d(
                        TAG,
                        "Skipping frame. Could not find ByteBuffer associated with the image data from the NucAction.camera."
                    )
                    return
                }
         ...
         *
         *
         * If detection takes longer than the time in between new frames from the NucAction.camera, this will
            ...
            while (true) {
                synchronized(lock) {
                    while (active && pendingFrameData == null) {
                        try {
                            // Wait for the next frame to be received from the NucAction.camera, since we don't have it yet.
                            lock.wait()
                        } catch (e: InterruptedException) {
                    ...
                    if (!active) {
                        // Exit the loop once this NucAction.camera source is stopped or released.  We check this here,
                    ...
                    // recycled back to the NucAction.camera before we are done using that data.
    ...
    companion object {
         ...
         * size and the picture size of the NucAction.camera together, because these need to have the same aspect
         * ratio. On some hardware, if you would only set the preview size, you will get a distorted
         * image.
         *
         * @param camera the NucAction.camera to select a preview size from
         ...
         *
         * @param camera the NucAction.camera to select a frames per second range from
         * @return the selected preview frames per second range
         */
        private fun selectPreviewFpsRange(camera: Camera): IntArray? {
            // The NucAction.camera API uses integers scaled by a factor of 1000 instead of floating-point frame
            // rates.
