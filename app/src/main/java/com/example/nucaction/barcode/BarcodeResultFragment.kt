...
class BarcodeResultFragment : BottomSheetDialogFragment(), View.OnClickListener {
    ...
    override fun onCreateView(
        ...
        val barcode: BarcodeField =
            arguments?.getParcelable<BarcodeField>(ARG_BARCODE_FIELD_LIST) as BarcodeField
        ...
        val btn_input_nutrition_info: Button =
            view.findViewById(R.id.barcode_field_btn_input_nutrition_info)
        ...
        when (v?.id) {
            R.id.barcode_field_btn_barcode_check -> {
                activity?.let {
                    ...
                    ViewModelProviders.of(it).get(WorkflowModel::class.java)
                        .setWorkflowState(WorkflowModel.WorkflowState.RECORDED)
                }
                getDialog()?.dismiss()
            }
            R.id.barcode_field_btn_input_nutrition_info -> {
                activity?.let {
                    // Back to working state after the bottom sheet is dismissed.
                    ViewModelProviders.of(it).get(WorkflowModel::class.java)
                        .setWorkflowState(WorkflowModel.WorkflowState.RECORD_BY_USER)
                }
                getDialog()?.dismiss()
            }
